...
export default function Messages({ user }: MessagesProps) {
  ...
  return (
    <div className="h-[calc(100vh-12rem)] flex gap-6 animate-in fade-in duration-500 font-sans">
      {/* Sidebar - Chat List (Admin Only) */}
      {isAdmin && (
        <Card className="w-80 border-none shadow-sm flex flex-col overflow-hidden bg-white">
          ...
          <CardContent className="p-0 flex-1 overflow-y-auto">
            {filteredChats.map((chat) => (
              ...
              >
                <div className="h-12 w-12 rounded-full bg-[#128c7e] flex items-center justify-center text-white font-bold text-lg">
      ...
      {/* Chat Window */}
      <Card className="flex-1 border-none shadow-sm flex flex-col overflow-hidden bg-[#e5ddd5]">
        {(!isAdmin || selectedStudent) ? (
          <>
            <CardHeader className="p-3 border-b border-slate-100 bg-[#075e54] text-white flex flex-row items-center space-x-3">
              ...
              <div className="flex-1">
                <CardTitle className="text-base">{isAdmin ? selectedStudent?.name : 'speakOral Support'}</CardTitle>
              ...
              >
                {messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      <div className={cn(
                        "p-2.5 px-4 rounded-xl shadow-sm text-sm relative",
              ...
              <div className="p-3 bg-[#f0f0f0] border-t border-slate-200 relative z-10">
                <form onSubmit={handleSend} className="flex items-center space-x-2">
                  <Input 
                    ...
                    className="rounded-full border-none bg-white h-11 px-6 focus-visible:ring-0 shadow-sm"
                    value={input}
